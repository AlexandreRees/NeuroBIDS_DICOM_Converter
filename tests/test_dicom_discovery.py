...
def test_data_ton_mixed_nii_and_deep_siemens(tmp_path: Path) -> None:
    """TON-like layout: shallow NIfTI leftover + deep Siemens PROTOCOL trees.

    Depth and markers matter more than real-world name length (Windows MAX_PATH)::

        Data_TON/
          SUBT01_Session1/…_NII/…
          SUBT01_Session02/…_DATA/ON02_STUDY-DST/DR_SHMUEL_PROTOCOL_A/series/
          SUBTON02-session1/ION1_STUDY-STD/DR_SHMUEL_PROTOCOL_B/series/
    """
    root = tmp_path / "Data_TON"

    # Session1 — leftover DICOM under *_NII (must not hide other subjects)
    _write_dicom(
        root / "SUBT01_Session1" / "SUBT01_Session1_NII" / "leftover.dcm",
        patient_id="ANON",
        series_uid=generate_uid(),
    )
    ...
    protocol_a = (
        root
        / "SUBT01_Session02"
        / "SUBT01_Session02_DATA"
        / "ON02_STUDY-DST-1107"
        / "DR_SHMUEL_PROTOCOL_A"
    )
    _write_dicom(
    ...
    assert any("DR_SHMUEL_PROTOCOL_A" in p for p in paths)
    assert any("DR_SHMUEL_PROTOCOL_B" in p for p in paths)
...
def test_batch_discovers_deep_siemens_under_data_ton(tmp_path: Path) -> None:
    root = tmp_path / "Data_TON"
    out = tmp_path / "bids"
    protocol = (
        root
    ...
    _write_dicom(
        root / "SUBT01_Session1" / "SUBT01_Session1_NII" / "x.dcm",
        patient_id="P01",
        series_uid=generate_uid(),
    )
    from neuro_pipeline.batch import BatchManager
