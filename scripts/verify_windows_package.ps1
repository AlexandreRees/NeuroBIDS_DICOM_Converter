...
if ($exeInfo.Length -lt 5MB) { throw "EXE unexpectedly small" }
if ($setupInfo.Length -lt 5MB) { throw "Setup.exe unexpectedly small" }
if ($dcmInfo.Length -lt 100KB) { throw "dcm2niix.exe unexpectedly small" }
...
try {
    ...
    if ($null -eq $proc) {
        ...
        Write-Host "SMOKE: EARLY_EXIT_HEADLESS_OR_GUI (exit=$exitCode) - not a hard failure on CI" -ForegroundColor Yellow
    }
} catch {
    $smokeStatus = "EXCEPTION"
    $errMsg = $_.Exception.Message
    Write-Host "SMOKE: EXCEPTION ($errMsg) - not a hard failure; file checks already passed" -ForegroundColor Yellow
}
...
try {
    ...
    if (($p.ExitCode -eq 0) -and (Test-Path $appExe) -and (Test-Path $appDcm)) {
        ...
        $exitCode = $p.ExitCode
        $appOk = Test-Path $appExe
        $dcmOk = Test-Path $appDcm
        Write-Host "SILENT_INSTALL: INCONCLUSIVE (exit=$exitCode; app=$appOk; dcm=$dcmOk)" -ForegroundColor Yellow
    ...
    $errMsg = $_.Exception.Message
    Write-Host "SILENT_INSTALL: FAILED_OR_UNSUPPORTED ($errMsg)" -ForegroundColor Yellow
    Write-Host "Manual install test on a real Windows desktop is still required before sending to PI." -ForegroundColor Yellow
} finally {
    try {
...
$dcmKb = [math]::Round($dcmInfo.Length / 1KB, 0)
Write-Host "  EXE:        $exe  ($exeMb MB) PE=OK"
Write-Host "  Setup:      $setup  ($setupMb MB) PE=OK"
Write-Host "  dcm2niix:   $dcmTools  ($dcmKb KB) PE=OK"
