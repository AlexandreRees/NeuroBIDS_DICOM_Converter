classdef CheckerFlickerSquare < handle
   properties
       tex
       stimParams
   ...
   methods (Access = public)
      function obj = CheckerFlickerSquare(checkerStimParams,flickerFrequency)
      ...
      function obj = createTextures(obj, diameter,screenProperties)
        ...
        for i =1:2:10
            rng('shuffle');
            an =2* pi* rand - pi;
            % Formula to create square checkers      
            switch obj.stimParams.gratingColor 
                    case colorBlackAndWhite
                        grating = zeros([size(R), 1]);
                        sinMatrix = obj.stimParams.amplitude*sign(sin(2*pi* cyclePerPixel * R)).*sign((sin((2*pi*obj.stimParams.cyclesPerRotation/(2*pi)*T)-an)));
                        grating(:,:,1) = round(obj.stimParams.mean*ones(size(R))+(sinMatrix));

                    case colorRedAndGreen
                        grating = zeros([size(R), 3]);
                        sinMatrix = sign(sin(2*pi* cyclePerPixel * R)) .* sign(sin((2*pi*obj.stimParams.cyclesPerRotation/(2*pi)*T) -an));
            ...
            switch obj.stimParams.gratingColor
                case colorBlackAndWhite
                        sinMatrix = obj.stimParams.amplitude*sign(-1*sin(2*pi* cyclePerPixel*R)).* sign(sin((2*pi*obj.stimParams.cyclesPerRotation/(2*pi)*T)-an));
                        grating(:,:,1) = round(obj.stimParams.mean * ones(size(R)) +(sinMatrix));
                    case colorRedAndGreen
