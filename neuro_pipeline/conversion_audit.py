...
def audit_series_conversion(
    *,
    ...
    if slice_thickness and len(voxel_size) >= 3:
        slice_ok = _approx_equal(slice_thickness, voxel_size[2], TOLERANCE_MM)
        checks.append(
            {
    ...
    if dicom_voxel_x and dicom_voxel_y and len(voxel_size) >= 2:
        xy_ok = (
            _approx_equal(dicom_voxel_x, voxel_size[0], TOLERANCE_MM)
            and _approx_equal(dicom_voxel_y, voxel_size[1], TOLERANCE_MM)
        )
        checks.append(
            {
                "name": "pixel_spacing",
    ...
    if sidecar_path.is_file():
        ...
        if tr_dicom and tr_json:
            tr_ok = _approx_equal(tr_dicom, tr_json, TOLERANCE_SEC)
            checks.append(
        ...
        if te_dicom and te_json:
            te_ok = _approx_equal(te_dicom, te_json, TOLERANCE_SEC)
            checks.append(
