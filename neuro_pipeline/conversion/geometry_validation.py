...
from neuro_pipeline.config.constants import GEOMETRY_VALIDATION_CSV_COLUMNS
...
def _approx_equal(a: float, b: float, tol: float = GEOMETRY_TOLERANCE_MM) -> bool:
    return abs(a - b) <= tol
...
def _orientation_consistent(dicom_iop: list[float], affine: np.ndarray, zooms: tuple[float, ...]) -> bool:
    ...
    return bool(np.all(diff <= GEOMETRY_ORIENTATION_TOLERANCE))
...
def validate_series_geometry(
    *,
    ...
    elif q_code > 0 and s_code > 0 and not np.allclose(qform, sform, atol=GEOMETRY_AFFINE_TOLERANCE):
...
def run_geometry_validation(
    ...
    *,
    ...
    if not conversion.is_file():
        ...
        pd.DataFrame(columns=list(GEOMETRY_VALIDATION_CSV_COLUMNS)).to_csv(
    ...
    pd.DataFrame([row.to_dict() for row in report.rows], columns=list(GEOMETRY_VALIDATION_CSV_COLUMNS)).to_csv(
