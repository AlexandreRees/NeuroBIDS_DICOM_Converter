...
from neuro_pipeline.reporting.generator import write_html_report
...
def _mriqc_summary_rows_by_modality(paths: ProjectPaths) -> dict[str, list[dict[str, Any]]]:
    """Load MRIQC IQM rows grouped by modality (no interpretation)."""
    summary_path = paths.mriqc_run_summary_json
    if not summary_path.is_file():
        return {}
    payload = _load_json(summary_path)
    by_modality = payload.get("mriqc_report_rows_by_modality", {})
    if isinstance(by_modality, dict) and by_modality:
        return {
            modality: [row for row in rows if isinstance(row, dict)]
            for modality, rows in by_modality.items()
            if isinstance(rows, list)
        }
    legacy_rows = payload.get("mriqc_report_rows", [])
    if isinstance(legacy_rows, list) and legacy_rows:
        grouped: dict[str, list[dict[str, Any]]] = {}
        for row in legacy_rows:
            if not isinstance(row, dict):
                continue
            modality = str(row.get("modality", row.get("suffix", "MRIQC")))
            grouped.setdefault(modality, []).append(row)
        return grouped
    return {}


def _mriqc_summary_rows(paths: ProjectPaths) -> list[dict[str, Any]]:
    """Load flattened MRIQC IQM rows for backward-compatible consumers."""
    rows_by_modality = _mriqc_summary_rows_by_modality(paths)
    flattened: list[dict[str, Any]] = []
    for modality in sorted(rows_by_modality):
        flattened.extend(rows_by_modality[modality])
    return flattened


def _mriqc_overview(paths: ProjectPaths) -> dict[str, str]:
    """Build high-level MRIQC execution facts for the summary table."""
    payload = _load_json(paths.mriqc_run_summary_json)
    if not payload:
        return {"MRIQC status": "not run"}
    config = payload.get("config", {})
    modalities = payload.get("modalities_detected", [])
    return {
        "MRIQC status": "enabled" if payload.get("run_mriqc", True) else "disabled",
        "MRIQC modalities detected": ", ".join(modalities) if modalities else "none",
        "MRIQC participants discovered": str(payload.get("participants_discovered", 0)),
        "MRIQC participants processed": str(payload.get("participants_processed", 0)),
        "MRIQC participants failed": str(payload.get("participants_failed", 0)),
        "MRIQC volumes discovered": str(payload.get("volumes_discovered", 0)),
        "MRIQC structural volumes": str(payload.get("structural_volumes_discovered", 0)),
        "MRIQC container runtime": str(config.get("container_runtime", "unknown")),
        "MRIQC container image": str(config.get("container_path", "unknown")),
        "MRIQC output directory": str(payload.get("output_directory", paths.mriqc_output_dir)),
    }
...
def build_qc_summary_payload(paths: ProjectPaths) -> dict[str, Any]:
    ...
    summary.update(_mriqc_overview(paths))
    mriqc_rows_by_modality = _mriqc_summary_rows_by_modality(paths)
    return {
        "summary": summary,
        "acquisition_rows": _acquisition_table(acquisition),
        "geometry_rows": _geometry_table(geometry),
        "mriqc_rows": _mriqc_summary_rows(paths),
        "mriqc_rows_by_modality": mriqc_rows_by_modality,
    }


def _mriqc_report_sections(payload: dict[str, Any]) -> list[tuple[str, list[dict[str, Any]]]]:
    """Build one HTML table section per detected MRIQC modality."""
    rows_by_modality = payload.get("mriqc_rows_by_modality", {})
    if isinstance(rows_by_modality, dict) and rows_by_modality:
        return [
            (f"MRIQC — {modality}", rows)
            for modality, rows in sorted(rows_by_modality.items())
            if isinstance(rows, list) and rows
        ]
    legacy_rows = payload.get("mriqc_rows", [])
    if isinstance(legacy_rows, list) and legacy_rows:
        return [("MRIQC Image Quality Metrics", legacy_rows)]
    return []
...
def generate_pipeline_qc_summary_html(paths: ProjectPaths) -> Path:
    ...
    sections.extend(_mriqc_report_sections(payload))
    write_html_report(
        output,
